...
CK=$(ls -1 runs/nanospeaker_grpo_split/checkpoints/rl_*.pt 2>/dev/null | tail -1)
...
python -m src.eval.bench --weights "$CK" --mode chat --limit 100 \
  ...
  --dump-items runs/items_split || echo "GATE FAILED"
...
for task in ("arith_gen", "humaneval"):
    ...
    new  = fails(f"runs/items_split/*_{task}_chat.json")
